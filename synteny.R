...
ref_data <- import.genome(fai_file="chrome12-50-PGA_assembly.fasta.fai")
query_data <- import.genome(fai_file="ch1-11-PGA_assembly.fasta.fai")
...
alignments2 <- subset(alignments, query_aln_prop[query] > 0.1)

#and finally we can visualise the alignemnt (see more examples of visualisation below)
plot.alignments.multi(alignments2, reference_lens=ref_data$seq_len, query_lens=query_data$seq_len, sigmoid=T)
